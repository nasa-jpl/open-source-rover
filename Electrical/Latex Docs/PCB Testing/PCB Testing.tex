...
\begin{enumerate}
	...
	\item Apply +5V onto pin 10 and GND onto pin 12. Verify that there is +5V across pin 22 and 24.
		\begin{figure}[H]
			...
			\label{vd7}
		\end{figure}
	\item Now that we know that each of the encoders will be powered by +5V correctly, we need to test the voltage division. Apply +5V to pin 14 and measure the output at pin 2 as shown in Figure \ref{vd8}. This value should read between 1.4V and 2.0V. It is important that it is strictly less than 2V, but slightly lower values are also okay. Resistor values can be changed if desired to make this closer to 2V. 
		\begin{figure}[H]
		 	\centering
