...
Now that the Op-amps are installed we want to check and make sure the voltage dividers are working correctly. To do this Jumper between the we will use the 5V line for the power of the motors on each block and feed it back into the analog read signal, and see what voltage it gets divided down to. On the motor connectors for each corner motor (J23-26) use a jumper wire and connect the 5V signal line to the ENx line. Then using a DMM measure the voltage between each of the following test pads and GND and compare the values to the expected voltage ranges below.
...
\begin{table}[H]
	...
	\begin{tabular}{| l | l | l |}
		\hline
		\thead{Signal} & \thead{Test Pad} & \thead{Voltage (V)} \\ \hline
