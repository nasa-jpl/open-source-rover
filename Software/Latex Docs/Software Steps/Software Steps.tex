...
\noindent Use nano (or your editor of choice) to add the above three 'source' commands to the end of your ~/.bashrc file, each on its own line.  This will allow you to re-launch the OSR code without needing to source the OSR ROS environment each time:
\begin{itemize}
	\item[] \colorbox{lightgray}{sudo nano ~/.bashrc}
\end{itemize}
...
\begin{itemize}
	\item[] Move to your built catkin workspace: \colorbox{lightgray}{cd /home/pi/osr\_ws/}
...
\begin{figure}[H]
	\centering
   \includegraphics[width=.7\textwidth]{"Pictures/ROS_architecture".PNG}
